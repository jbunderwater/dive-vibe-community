---
name: Davíðsgjá Fissure
lat: 64.26
lng: -21.1
difficulty: Intermediate
maxDepth: 21
entryType: shore
siteType: wall
ref: null
osmId: null
addedBy: osm_import
---

## Davíðsgjá Fissure

Davíðsgjá is a tectonic fissure dive in Þingvellir National Park — less visited than Silfra, it offers the same glacial clarity in a narrower, more atmospheric setting that local divers describe as Silfra's "darker, spookier sibling."

## Overview

Davíðsgjá is a submerged rift fissure in Lake Þingvallavatn within Þingvellir National Park, located approximately 4 kilometres from the famous Silfra fissure (about 10 minutes by car). Like Silfra, it lies on the boundary between the North American and Eurasian tectonic plates and is filled with glacially sourced freshwater of exceptional clarity.

The fissure's top begins approximately 7 metres below the lake surface, reached after a shallow swim from shore. Divers enter the calm, open water of the lake and swim to the fissure entry before descending into the crack. The maximum depth is 21 metres. Further into the fissure the passage branches: a narrow, dark gorge to the left and a wider, more open gorge to the right — a feature that distinguishes Davíðsgjá from the single-channel Silfra system.

The site is considerably less crowded than Silfra and is described by DIVE.IS as a favourite among local Icelandic divers. The visual character differs from Silfra: the fissure walls are darker and the geometry more enclosed, with a more cave-like atmosphere in the narrower branch. However, it is not a cave dive — both branches are open tectonic fissures.

Water conditions differ from Silfra: temperature is typically slightly warmer than the 2–4°C of Silfra (both are near-freezing), and visibility, while often excellent, is more dependent on weather and lake conditions than the spring-fed Silfra. Drysuit is mandatory; no wetsuit diving is permitted.

## Dive Profile

- **Entry**: Shore entry. Accounts vary by source and likely by season/ice conditions: DIVE.IS and Adventures.is describe walking straight in for a shallow swim to the fissure opening; Divers-Guide.com describes a roughly 300-metre walk over a rocky bank to the entry point; the PADI blog describes a winter dive with a "giant stride entry from an icy rock."
- **Maximum depth**: Sources disagree — DIVE.IS and the PADI blog both give 21 m, while Divers-Guide.com lists 15 m. The two independent operator/publisher sources agreeing on 21 m are treated as the more reliable figure here, but the discrepancy has not been fully resolved.
- **Fissure top**: Approximately 7 m below lake surface (DIVE.IS)
- **Dive duration**: 30–40 minutes typically
- **Visibility**: Typically 100 m+ (DIVE.IS, Adventures.is); Divers-Guide.com reports a range of 100–300 m with an average around 200 m. More weather-dependent than Silfra.

## Requirements

- **Mandatory equipment**: Drysuit (wetsuits prohibited)
- **Certification**: Open Water minimum; drysuit certification or 10 logged drysuit dives required
- **Guided dive**: Required; access through accredited operators only
- **Permit**: Þingvellir National Park dive permit required
- **Winter access**: Road to site may require 4x4; ice-ceiling conditions possible in winter, which requires Ice Diver certification

## Marine Life

Marine life is sparse, as in all Icelandic freshwater dive sites. Brown trout are documented at the site by DIVE.IS — they are commonly encountered swimming near the fissure opening. Þingvallavatn Lake is home to three fish species in total (Þingvellir National Park's own "Fish in the Lake" page): brown trout, Arctic char (which occurs in four distinct morphs, including a dwarf morph), and three-spined stickleback. Dwarf char, one of those Arctic char morphs, live in the lake's crevices generally, but no dive-specific source reviewed confirms dwarf char sightings specifically within the Davíðsgjá fissure itself. Note: an earlier version of this description used the scientific name "Salvelinus devarius" for dwarf char — this name does not appear in any source reviewed (including Þingvellir National Park's own materials) and has been removed as unverified/fabricated. The dwarf morph is a form of Arctic char (Salvelinus alpinus); a separate name, "Salvelinus thingvallensis," appears on some third-party taxonomy sites but is not used by the National Park itself, so it is omitted here pending better sourcing. The primary visual draw at Davíðsgjá is geological: the fissure walls, tectonic geometry, and glacial water colour.

## Photography

The narrower geometry and darker walls of Davíðsgjá create a different photographic character from Silfra — more dramatic shadow and depth contrasts. The branching structure at depth provides compositional variety for wide-angle shooting.

## Safety

Water temperature of 2–5°C requires a high-quality drysuit with adequate thermal underlayers. The shore entry involves swimming across open lake water in full diving equipment, which requires good physical condition. Always dive with a guide and partner. Winter dives with ice overhead require additional certification and equipment.

## Site Information

- **Location**: Þingvellir National Park, Iceland
- **Entry Type**: Shore entry
- **Site Type**: Wall dive
- **Difficulty Level**: Intermediate
- **Maximum Depth**: 21 meters
- **Protected Status**: UNESCO World Heritage Site (Þingvellir National Park)

---
*Sources: [DIVE.IS — Davíðsgjá dive site](https://www.dive.is/dive-sites/davidsgja), [PADI — Diving Davíðsgjá](https://blog.padi.com/diving-davidsgja-silfras-darker-spookier-sibling/), [Adventures.is — Davíðsgjá](https://adventures.is/iceland/attractions/davidsgja/), [Divers-Guide — Davíðsgjá](https://www.divers-guide.com/en/davisgja), [Þingvellir National Park — Fish in the Lake](https://www.thingvellir.is/en/education/nature/fish-in-the-lake/). Re-audited 2026-09-18: fabricated species name "Salvelinus devarius" removed; depth and entry-distance conflicts across sources now flagged. Last updated 2026-09-18.*
