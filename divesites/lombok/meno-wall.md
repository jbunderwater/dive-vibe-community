---
name: Meno Wall
lat: -8.3452031
lng: 116.0498999
difficulty: Intermediate
maxDepth: 22
entryType: boat
siteType: wall
ref: null
osmId: 2395669756
addedBy: osm_import
---

## Meno Wall

A two-section coral wall on the northwest side of Gili Meno — one of the Gili Islands' best night dive sites, outstanding for turtles, bumphead parrotfish, and macro life.

## Overview

Meno Wall runs along the northwest side of Gili Meno, featuring two distinct wall sections: the first from 10–13 m, followed by a sandy ledge, then a second wall from 14–24 m. Current varies greatly — from non-existent to some of the strongest in the Gilis — making the site suitable for drift diving when current is running. Hawksbill, green, and leatherback turtles are present in large numbers and the site is sometimes called Turtle Heaven by local operators. Night dives here are excellent — crustaceans that shelter by day emerge at dark, and lobsters, shrimps, crabs, and Spanish dancer nudibranchs become the main attraction. The wall is covered in soft corals and anemones, with moray eels, lionfish, scorpionfish, grouper, and bumphead parrotfish (visible in the blue water off the wall) regularly encountered. This is a macro photographer's favourite among Gili Meno sites.

## Site Information

- **Location**: Northwest side of Gili Meno, Gili Islands / Lombok
- **Entry Type**: Boat dive
- **Site Type**: Wall dive
- **Difficulty Level**: Intermediate
- **Maximum Depth**: 22 meters
- **Typical Visibility**: 20 m and above
- **Current**: Highly variable — can be none or very strong
- **Best Time**: May to October

## Marine Life

Hawksbill, green, and leatherback turtles in large numbers — a standout site for turtle encounters. Moray eels and scorpionfish, confirmed by two independent sources (Trawangan Dive and Gili Divers) as of the 2026-08-11 re-audit, along with lionfish, pipefish, and grouper in wall crevices and overhangs. Sponges, nudibranchs, squid, and decorator crabs on the wall face. Bannerfish, batfish, triggerfish, sweetlips, snappers, angelfish, and fusiliers throughout. Bumphead parrotfish confirmed, visible in the blue off the wall. Night diving is excellent — red shrimps, squid, lobsters, crabs, and Spanish dancer nudibranchs emerge after dark.

Bamboo sharks at this site are not confirmed by any reviewed source. Pygmy seahorses are not confirmed at this site by reviewed sources.

## Dive Profile

Descend to 15–20 m on the wall and begin drifting when current allows. Follow the wall face upward progressively. Concentrate on the 5–15 m zone during the final third of the dive where turtle and fish density peaks. Complete a 3-minute safety stop in shallow water above the wall.

## Entry and Exit

Boat from Gili Meno or Gili Trawangan operators. Check current conditions before entry. SMB mandatory for all dives, essential when current is running.

## Tips and Recommendations

- Night dives here are among the best in the Gili Islands — book with an operator who runs night boat trips
- Current is highly variable — conditions must be checked before entry
- Combine with the Bounty Wreck for a complete Gili Meno diving day
- The macro life in the wall crevices rewards a slow, detailed approach

## Safety Considerations

Current can accelerate suddenly and is unpredictable. Ascend to the shallower wall if current becomes too strong. Always carry an SMB. Night dives require each diver to carry a primary light and a backup. Be aware of boat traffic in the channel especially at night.

## Photography

Night dive photography reveals fully coloured lobsters, shrimps, and Spanish dancers. Day dives suit wide-angle for the soft coral wall and passing turtles. Macro lens for nudibranchs and wall crevice detail.

---
*Sources: [Manta Dive Gili Air – Meno Wall](https://manta-dive-giliair.com/gili-islands-dive-sites/meno-wall-all-levels), [Trawangan Dive – Gili Dive Sites](https://www.trawangandive.com/dive-sites-gili-islands/), [Gili Divers – Dive Sites](https://www.gilidivers.com/fun-dives/dive-sites), [Terumbu Divers – Gili Dive Sites](https://terumbudivers.com/fun-dives-2/dive-sites-gili-islands-lombok/). Last updated 2026-08-11.*
