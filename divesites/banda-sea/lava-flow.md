---
name: Lava Flow
lat: -4.5
lng: 129.87
difficulty: Intermediate
maxDepth: 30
entryType: boat
siteType: reef
ref: null
osmId: null
addedBy: osm_import
---

## Lava Flow

Lava Flow is a coral reef site on the northern flank of Gunung Api, the active volcano that rises from the Banda Sea. The 1988 eruption sent lava cascading into the ocean, and the coral recovery on the volcanic substrate is one of the most celebrated reef regeneration stories in the region. Scientists have documented some table corals here growing to over 4 metres in diameter in under 10 years — a rate attributed in part to the nutrient content of volcanic ash.

## Overview

The black volcanic substrate has been colonised by exceptionally fast-growing coral. The site now showcases staghorn, plate, table, and cabbage-leaf corals at densities that have drawn scientific study. Table corals grow to impressive proportions across the shallower sections, while deeper areas (20–30 m) feature fusilier aggregations. The dark lava rock creates a striking visual contrast against colourful coral growth.

Sources disagree on whether mandarinfish are present at Lava Flow itself: All Star Liveaboards describes Lava Flow as a mandarinfish location, but Bottles Up Diving, Merapi Tours, Dive The World, and A Lovely Planet each describe the reef's fish life without mentioning mandarinfish, and instead identify Mandarin City — a separate, shallow site in front of Hotel Maulana on Banda Neira's harbour front, several kilometres away — as the primary place to see the mating display. Given this weight of evidence, the mandarinfish mating display should be treated as a Mandarin City phenomenon rather than a confirmed Lava Flow feature. Napoleon wrasse, batfish, and mobula rays are reported at Lava Flow itself.

## Dive Profile

A gently sloping reef from the shallows to around 30 m; sources vary and report maximum depths anywhere from 20 to 35 m. The shallower sections (3–15 m) hold the densest coral formations. Deeper areas transition to more open substrate with scattered coral blocks and fusilier schools. The site is accessible as both a day dive from Banda Neira and as a liveaboard stop.

## Marine Life

- Napoleon wrasse, batfish, mobula rays
- Red-tooth triggerfish, parrotfish
- Dense fusilier schools
- Cuttlefish

## Entry / Exit

Boat dive from liveaboard or from Banda Neira-based operators. The site is within reach of day-trip boats from Banda Neira.

## Tips

- Shallow sections (3–10 m) are ideal for snorkellers and beginner divers — the lava flow topography rewards slow exploration
- For a reliable mandarinfish dusk dive, plan the separate trip to Mandarin City in Banda Neira harbour rather than expecting them here

## Photography

One of the Banda Sea's premier wide-angle reef photography sites — the black lava contrasts vividly with colourful coral formations.

---
*Sources: [Bottles Up Diving - Banda Islands](https://www.bottles-up-diving.com/EN-pages/BU_divingother_banda_EN.html), [All Star Liveaboards - Banda Sea Guide](https://allstarliveaboards.com/banda-sea-dive-liveaboard-forgotten-islands/), [Merapi Tours - Banda Archipelago Dive Spots](https://www.merapitours.com/indonesia/moluccas/dive-spots-banda-archipelago), [A Lovely Planet - Banda Islands Diving Guide](https://www.alovelyplanet.com/banda-islands-diving-guide/), [Dive The World - Banda Islands](https://www.dive-the-world.com/diving-sites-indonesia-banda-islands.php). Last updated 2026-07-13.*
